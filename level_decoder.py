...
def token_to_group(tensor, tokens, token_groups=TOKEN_GROUPS):
    ...
    new_tensor = torch.zeros(tensor.shape[0], len(token_groups), *tensor.shape[2:]).to(tensor.device)
    for i, token in enumerate(tokens):
        for group_idx, group in enumerate(token_groups):
            if token in group:
    ...
    for k in range(len(tokens)):
        for i in range(len(level)):
            for j in range(len(level[-1])):
                token = level[i][j]
                if token in tokens[k] and token != "\n":
                    oh_level[k, i, j] = 1
    return oh_level
...
def one_hot_to_ascii_level(level, tokens):
    ...
    for i in range(level.shape[1]):
        ...
        for j in range(level.shape[2]):
            line += list(tokens[level[:, i, j].argmax()].keys())[0] #0 to default take first token from group
...
if __name__ == "__main__":
    img_path = 'resources/levels/mario/'
    ImgGen = LevelImageGen(img_path+'sprites')
    file = load_level_from_text('resources/levels/mario/lvl_1-1.txt')
    onehot = ascii_to_one_hot_level(file, tokens=TOKEN_GROUPS)
    torch.set_printoptions(edgeitems=300)
    ascii= one_hot_to_ascii_level(onehot, tokens= TOKEN_GROUPS)
    img = ImgGen.render(ascii)
    img.save(img_path+'test.png', format='png')
    print(ascii)
    pass
